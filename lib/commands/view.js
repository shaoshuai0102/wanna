...
function loadViewEngine(callback) {
    ...
    npm.load({prefix: process.env._ + '/../../'}, function(err, data) {
        if (err) {
            console.error('Install template engine error: ', err);
            process.exit(1);
        }
        ...
        npm.commands.install([str], function(err, data) {
            ...
            console.log('Install template engine success.');
            callback();
        });
...
function viewEngineLoaded() {
    return fs.existsSync(process.env._ + '/../../node_modules/' + config_viewEngine.name)
}
...
function render(layout, data, target) {
    ...
    consolidate[config_viewEngine.name](layout, data, function(err, html){
        if (err) throw err;
        //console.log(html);
